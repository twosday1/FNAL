...
public class StormyAction : MonoBehaviour
{
    ...
    [Tooltip("Multiplier applied to movement interval for each extra level (0..1). Values <1 make Stormy faster per level.")]
    ...
    [Tooltip("Seconds it takes Stormy to move between spots.")]
    [SerializeField] private float moveDuration = 0.5f;

    [Header("Door / kill settings")]
    [Tooltip("Seconds Stormy waits at the door before killing the player (unless flashed).")]
    [SerializeField] private float doorKillDelay = 6f;
    [Tooltip("Number of flashlight flashes required to reset Stormy when at the door.")]
    ...
    [Tooltip("Allow RegisterFlash() to succeed even when Stormy is not exactly at the door (useful for testing).")]
    ...
    // Controls whether Stormy is allowed to perform movement attempts.
    ...
    public void OnLevelStart()
    {
        ...
        if (debugLogs) Debug.Log($"[StormyAction] OnLevelStart: movement enabled (interval={currentMovementInterval:F2}s)");
    }

    // Called by LevelProgression when level is passed or player dies — reset Stormy and prevent movement until next level start
    public void ResetAndDisableMovement()
    {
        if (debugLogs) Debug.Log("[StormyAction] ResetAndDisableMovement: resetting and disabling movement");
        // Stop any current coroutines that would be moving Stormy
        StopAllCoroutines();

        // Reset state
        ValidateAndInit(); // places Stormy at start and resets flashCount
        moving = false;
    ...
    private void ApplyLevelScaling()
    {
        ...
        if (debugLogs)
        {
            Debug.Log($"[StormyAction] ApplyLevelScaling: level={level}, startChance={startMoveChance:F3}, forwardChance={forwardMoveChance:F3}, backwardChance={backwardMoveChance:F3}, movementInterval={currentMovementInterval:F2}");
        }
    }
    ...
    private void ValidateAndInit()
    {
        ...
        {
            Debug.LogError("[StormyAction] Please assign at least 4 movement spots in the Inspector (Start, Living1, Living2, Door).");
            enabled = false;
            return;
        }
        ...
        // Place Stormy at the configured start immediately
    ...
    private IEnumerator MainBehaviorLoop()
    {
        ...
        while (true)
        {
            ...
            {
                if (debugLogs) Debug.Log("[StormyAction] Reached door. Starting door sequence.");
                yield return StartCoroutine(DoorSequence());
                // After door sequence ends (reset or kill), if Stormy was reset to start, continue loop.
                continue;
            }
            ...
            if (currentIndex == startIndex)
            {
                ...
                if (Random.value < startMoveChance)
                {
                    ...
                    if (debugLogs) Debug.Log($"[StormyAction] Start -> moving to index {target}");
                    yield return StartCoroutine(MoveToIndex(target));
                }
                else
                {
                    if (debugLogs) Debug.Log("[StormyAction] Start -> did not get the move roll");
                }
            }
            else
            {
                ...
                if (currentIndex < doorIndex && Random.value < forwardMoveChance)
                {
                    ...
                    if (target == doorIndex)
                    {
                        ...
                        {
                            if (debugLogs) Debug.Log("[StormyAction] Cannot move to door: Lilly is already at the door.");
                            // Do not move to the door
                            continue;
                        }
                    }

                    if (debugLogs) Debug.Log($"[StormyAction] Middle -> forward to {target}");
                    yield return StartCoroutine(MoveToIndex(target));
                    moved = true;
                }
                ...
                if (!moved && currentIndex > startIndex && Random.value < backwardMoveChance)
                {
                    ...
                    if (debugLogs) Debug.Log($"[StormyAction] Middle -> backward to {target}");
                    yield return StartCoroutine(MoveToIndex(target));
                    moved = true;
                }

                if (!moved && debugLogs) Debug.Log("[StormyAction] Middle -> no move this opportunity");
            }
    ...
    private IEnumerator MoveToIndex(int targetIndex)
    {
        ...
        while (elapsed < moveDuration)
        {
            ...
            if (!movementEnabled)
            {
                if (debugLogs) Debug.Log("[StormyAction] MoveToIndex cancelled because movementDisabled");
                moving = false;
                yield break;
            }
    ...
    private IEnumerator DoorSequence()
    {
        ...
        if (debugLogs) Debug.Log("[StormyAction] DoorSequence: waiting for flashes");
        ...
        while (elapsed < doorKillDelay)
        {
            ...
            {
                if (debugLogs) Debug.Log("[StormyAction] DoorSequence: flashed enough -> resetting to start");
                ResetToStart();
                yield break;
            }
            ...
            if (!movementEnabled)
            {
                if (debugLogs) Debug.Log("[StormyAction] DoorSequence: exiting because movement disabled");
                yield break;
            }
        ...
        if (flashCount >= requiredFlashesToReset)
        {
            if (debugLogs) Debug.Log("[StormyAction] DoorSequence: flashed enough before timeout -> resetting to start");
            ResetToStart();
            yield break;
        }

        if (debugLogs) Debug.Log("[StormyAction] DoorSequence: not enough flashes -> killing player");
        KillPlayer();
        yield break;
    }

    // Public method the player's flashlight code should call when it hits Stormy
    public void RegisterFlash()
    {
        // Helpful debug info when flashes are being attempted
        if (debugLogs)
            Debug.Log($"[StormyAction] RegisterFlash called (currentIndex={currentIndex}, doorIndex={doorIndex}, flashCount={flashCount})");

        if (currentIndex != doorIndex && !allowFlashWhenNotAtDoor)
        {
            if (debugLogs) Debug.Log($"[StormyAction] RegisterFlash ignored: Stormy not at door (index {currentIndex}).");
            return; // Only count flashes at the door unless testing override is enabled
        }

        flashCount++;
        if (debugLogs) Debug.Log($"[StormyAction] RegisterFlash: flashCount = {flashCount}");
    ...
    {
        if (debugLogs) Debug.Log("[StormyAction] ForceFlash invoked from inspector (bypasses door check).");
        flashCount++;
    }
    ...
    private void ResetToStart()
    {
        ...
        flashCount = 0;
        moving = false;
        if (debugLogs) Debug.Log("[StormyAction] ResetToStart: Stormy returned to start");
    ...
    private void KillPlayer()
    {
        ...
        // Fall back to reloading the scene if LevelProgression is not present.
        if (debugLogs) Debug.Log("[StormyAction] KillPlayer: notifying LevelProgression or reloading scene");
        ...
        {
            if (debugLogs) Debug.Log("[StormyAction] KillPlayer: LevelProgression not found, reloading active scene");
            SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);
        }
    }

    // Optional helper: allow forcing Stormy to a specific spot index at runtime
